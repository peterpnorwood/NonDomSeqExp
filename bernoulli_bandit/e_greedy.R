...
## Purpose: simulate an experiment with e-greedy randomization ----- ##
## ----------------------------------------------------------------- ##
...
e_greedy <- function(train_set,burn_in,N,
  ...
  while(num_subjects<N){
    ...
    prob_vec <- rep(eps/(num_options-1),num_options)
    prob_vec[which.max(kl_info[,2])] <- 1-eps
    
    combo <- sample(1:2**num_new,1,prob=prob_vec)
...
#test <- e_greedy(train_set=train_set,burn_in=20,N=95,
