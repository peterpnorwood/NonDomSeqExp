...
## randomization where the 1/tr(XtX)^{-1} is the info gain metric -- ##
## ----------------------------------------------------------------- ##
...
## tr_eps_greedy
## Purpose: run a trial with restricted eps-greedy
...
tr_eps_greedy <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    tr_info$rand_prob <- NA
    if(length(max_info)==1){
      tr_info$rand_prob <- ifelse(tr_info$A == tr_info$A[which.max(tr_info$yhat)],1,0)
    }else{
      for(k in 1:4){
        if(tr_info$A[k] == tr_info$A[which.max(tr_info$yhat)]){
          tr_info$rand_prob[k] <- 1-eps
        }else if(tr_info$A[k] %in% max_info){
          tr_info$rand_prob[k] <- eps/(length(max_info)-1)
        } else{
          tr_info$rand_prob[k] <- 0
        }
      }
    }
    ...
    trial[i,4] <- sample(0:3,1,prob=tr_info$rand_prob)
...
#                        sigma=0.1)
# 
# test <- tr_eps_greedy(train_set,burn_in=50,N=nrow(train_set),
#                beta00=beta00,beta10=beta10,beta20=beta20,beta30=beta30,
#                beta01=beta01,beta11=beta11,beta21=beta21,beta31=beta31,
#                beta02=beta02,beta12=beta12,beta22=beta22,beta32=beta32,
#                beta03=beta03,beta13=beta13,beta23=beta23,beta33=beta33,
