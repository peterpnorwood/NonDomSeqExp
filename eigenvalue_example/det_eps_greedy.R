...
## Purpose: simulate an experiment with restricted e-greedy -------- ##
## randomization where det XtX is the info gain metric ------------- ##
## ----------------------------------------------------------------- ##
...
## det_eps_greedy
## Purpose: run a trial with restricted e-greedy
...
det_eps_greedy <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    det_info <- matrix(NA,nrow=4,ncol=4)
    for(a in 0:3){
      ...
      ## add onto det_info
      det_info[a+1,] <- c(a,yhat,mu,det_XtX)
      
    }
    
    ## finding the treatmetns which maximize a convex combo
    ## minus one to index these correctly
    max_info <- comb(x=det_info[,2],y=det_info[,4])-1
    true_non_dom <- comb(x=det_info[,3],y=det_info[,4])-1
    
    
    det_info <- as.data.frame(det_info)
    colnames(det_info) <- c("A","yhat","mu","info_gain")
    det_info$rand_prob <- NA
    if(length(max_info)==1){
      det_info$rand_prob <- ifelse(det_info$A == det_info$A[which.max(det_info$yhat)],1,0)
    }else{
      for(k in 1:4){
        if(det_info$A[k] == det_info$A[which.max(det_info$yhat)]){
          det_info$rand_prob[k] <- 1-eps
        }else if(det_info$A[k] %in% max_info){
          det_info$rand_prob[k] <- eps/(length(max_info)-1)
        } else{
          det_info$rand_prob[k] <- 0
        }
      }
    }
    ...
    trial[i,4] <- sample(0:3,1,prob=det_info$rand_prob)
    ...
    trial[i,8] <- ifelse(trial[i,4]==det_info$A[which.max(det_info$mu)],1,0)
...
#                        sigma=0.1)
# 
# test <- det_eps_greedy(train_set,burn_in=50,N=nrow(train_set),
#                beta00=beta00,beta10=beta10,beta20=beta20,beta30=beta30,
#                beta01=beta01,beta11=beta11,beta21=beta21,beta31=beta31,
#                beta02=beta02,beta12=beta12,beta22=beta22,beta32=beta32,
#                beta03=beta03,beta13=beta13,beta23=beta23,beta33=beta33,
