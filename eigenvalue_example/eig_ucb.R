...
## where the min eigenvalue is the info gain metric ---------------- ##
## ----------------------------------------------------------------- ##
...
eig_ucb <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    eig_info <- matrix(NA,nrow=4,ncol=5)
    for(a in 0:3){
      ...
      ## add onto det_info
      eig_info[a+1,] <- c(a,yhat[1],mu,min(eigs$values),yhat[3])
      
    }
    
    eig_info <- as.data.frame(eig_info)
    colnames(eig_info) <- c("A","yhat","mu","info_gain","ucb")
    
    ## finding the treatmetns which maximize a convex combo
    ## minus one to index these correctly
    non_dom <- comb(x=eig_info[,2],y=eig_info[,4])-1
    eig_info$non_dom <- ifelse(eig_info$A %in% non_dom,1,0)
    true_non_dom <- comb(x=eig_info[,3],y=eig_info[,4])-1
    
    trial[i,1] <- i
    ## new treatment for subject
    trial[i,4] <- eig_info[eig_info$non_dom==1,]$A[which.max(eig_info[eig_info$non_dom==1,]$ucb)]
