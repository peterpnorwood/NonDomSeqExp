...
## where the det XtX is the info gain metric ----------------------- ##
## ----------------------------------------------------------------- ##
...
det_ucb <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    det_info <- matrix(NA,nrow=4,ncol=5)
    for(a in 0:3){
      ...
      ## add onto det_info
      det_info[a+1,] <- c(a,yhat[1],mu,det_XtX,yhat[3])
      
    }
    
    det_info <- as.data.frame(det_info)
    colnames(det_info) <- c("A","yhat","mu","info_gain","ucb")
    
    ## finding the treatmetns which maximize a convex combo
    ## minus one to index these correctly
    non_dom <- comb(x=det_info[,2],y=det_info[,4])-1
    det_info$non_dom <- ifelse(det_info$A %in% non_dom,1,0)
    true_non_dom <- comb(x=det_info[,3],y=det_info[,4])-1
    
    trial[i,1] <- i
    ## new treatment for subject
    trial[i,4] <- det_info[det_info$non_dom==1,]$A[which.max(det_info[det_info$non_dom==1,]$ucb)]
