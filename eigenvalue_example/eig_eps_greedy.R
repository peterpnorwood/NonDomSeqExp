...
## Purpose: simulate an experiment with restricted eps-greedy ------ ##
...
eig_eps_greedy <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    eig_info$rand_prob <- NA
    if(length(max_info)==1){
      eig_info$rand_prob <- ifelse(eig_info$A == eig_info$A[which.max(eig_info$yhat)],1,0)
    }else{
      for(k in 1:4){
        if(eig_info$A[k] == eig_info$A[which.max(eig_info$yhat)]){
          eig_info$rand_prob[k] <- 1-eps
        }else if(eig_info$A[k] %in% max_info){
          eig_info$rand_prob[k] <- eps/(length(max_info)-1)
        } else{
          eig_info$rand_prob[k] <- 0
        }
      }
    }
...
#                        sigma=0.1)
# 
# test <- eig_eps_greedy(train_set,burn_in=50,N=nrow(train_set),
#                beta00=beta00,beta10=beta10,beta20=beta20,beta30=beta30,
#                beta01=beta01,beta11=beta11,beta21=beta21,beta31=beta31,
#                beta02=beta02,beta12=beta12,beta22=beta22,beta32=beta32,
#                beta03=beta03,beta13=beta13,beta23=beta23,beta33=beta33,
