...
## Purpose: simulate an experiment with thompson sampling ---------- ##
...
eig_thompson <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    eig_info$A <- as.character(eig_info$A)
    thomp_probs$A <- as.character(thomp_probs$A)
    eig_info <- dplyr::left_join(eig_info,thomp_probs,by="A")
    eig_info$probs <- ifelse(is.na(eig_info$probs),0, eig_info$probs)
    
    ## save patient information
    trial[i,1] <- i
    ## new treatment for subject
    trial[i,4] <- sample(0:3,1,prob=eig_info$probs)
...
#                        sigma=0.1)
# 
# test <- eig_thompson(train_set,burn_in=50,N=nrow(train_set),
#                beta00=beta00,beta10=beta10,beta20=beta20,beta30=beta30,
#                beta01=beta01,beta11=beta11,beta21=beta21,beta31=beta31,
#                beta02=beta02,beta12=beta12,beta22=beta22,beta32=beta32,
#                beta03=beta03,beta13=beta13,beta23=beta23,beta33=beta33,
