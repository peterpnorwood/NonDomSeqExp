...
## sampling where the det XtX is the info gain metric -------------- ##
## ----------------------------------------------------------------- ##
...
det_thompson <- function(train_set,burn_in,N,
  ...
  for(i in (burn_in+1):N){
    ...
    det_info$A <- as.character(det_info$A)
    thomp_probs$A <- as.character(thomp_probs$A)
    det_info <- dplyr::left_join(det_info,thomp_probs,by="A")
    det_info$probs <- ifelse(is.na(det_info$probs),0, det_info$probs)
    
    ## save patient information
    trial[i,1] <- i
    ## new treatment for subject
    trial[i,4] <- sample(0:3,1,prob=det_info$probs)
...
#                        sigma=0.1)
# 
# test <- det_thompson(train_set,burn_in=50,N=nrow(train_set),
#                beta00=beta00,beta10=beta10,beta20=beta20,beta30=beta30,
#                beta01=beta01,beta11=beta11,beta21=beta21,beta31=beta31,
#                beta02=beta02,beta12=beta12,beta22=beta22,beta32=beta32,
#                beta03=beta03,beta13=beta13,beta23=beta23,beta33=beta33,
