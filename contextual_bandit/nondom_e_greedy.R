...
## Purpose: run an experiment with non-dominated ------------------- ##
...
## nondom_e_greedy
## Purpose: run a non-dominated e_greedy experiment
...
nondom_e_greedy <- function(train_set,burn_in,A,theta,sd_Y,eps){
  ...
  for(i in (burn_in+1):N){
    ...
    prob_vec <- rep(0,length(A))
    prob_vec[est_nondom] <- rep(eps/(length(est_nondom)-1),length(est_nondom))
