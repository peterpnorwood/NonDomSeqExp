...
## Purpose: run an experiment nondom ucb selecction ---------------- ##
## ----------------------------------------------------------------- ##
...
## nondom_ucb
## Purpose: run an experiment with nondom ucb selection
...
## param percentile: percentile on the upper confidence bound
## return dat: dataframe with X,A,mu,Y,regret,norm
nondom_ucb <- function(train_set,burn_in,A,theta,sd_Y,percentile){
  ...
  for(i in (burn_in+1):N){
    ...
    info <- matrix(NA,nrow=length(A),ncol=5)
    tick=1
    for(a in A){
      ## gather ests if a is assigned
      temp_dat <- data.frame(t(dat[i,1:p]),A=a,Y=0)
      ## estiamted mean outcome and upper confidence bound given a
      pred_info <- predict(fit,temp_dat,interval = "prediction",alpha=1-percentile)
      mu_hat <- pred_info[1]
      upp_cb <- pred_info[3]
      ...
      ## save info
      info[tick,] <- c(a,mu_hat,mu,upp_cb,info_gain)
      tick=tick+1
    }
    ## save info as dataframe
    info <- data.frame(info)
    colnames(info) <- c("A","mu_hat","mu","ucb","info_gain")
    
    ## true non-dominated
    true_nondom <- comb(info$mu,info$info_gain)
    ## estimated non-dominated 
    est_nondom <- comb(info$mu_hat,info$info_gain)
    
    ## nondom info
    nondom_info <- info[info$A %in% est_nondom,]
    
    ## assign intervention (e-greedy)
    dat[i,p+1] <- nondom_info$A[which.max(nondom_info$ucb)]
    ## find mean outcome
