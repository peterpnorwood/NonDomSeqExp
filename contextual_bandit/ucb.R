...
## Purpose: run an experiment ucb selecction ----------------------- ##
## ----------------------------------------------------------------- ##
...
## ucb
## Purpose: run an experiment with ucb selection
...
ucb <- function(train_set,burn_in,A,theta,sd_Y,percentile){
  ...
  for(i in (burn_in+1):N){
    ...
    dat[i,p+1] <- info$A[which.max(info$ucb)]
    ## find mean outcome
